...
func (h *Handler) Discover(w http.ResponseWriter, r *http.Request) {
	...
	for i, device := range devices {
		...
		// some device types (e.g. HubMini2 / "Hub Mini2") may not be defined as a
		// constant in the go-switchbot package. Accept known string names as a
		// fallback so these hubs aren't ignored.
		if !deviceTypeIsSupported {
			t := strings.TrimSpace(string(device.Type))
			if t != "HubMini2" && t != "Hub Mini2" {
...
func (h *Handler) Metrics(w http.ResponseWriter, r *http.Request) {
	...
	for _, target := range targets {
		...
		default:
			// Some device types (like HubMini2 / "Hub Mini2") might not have a
			// constant in the used go-switchbot version. Detect them by string and
			// handle as Hub2.
			t := strings.TrimSpace(string(status.Type))
			if t == "HubMini2" || t == "Hub Mini2" {
				log.Printf("device is a meter-ish device (HubMini2 detected by string)")
				meterHumidity.WithLabelValues(status.ID).Set(float64(status.Humidity))
				meterTemperature.WithLabelValues(status.ID).Set(status.Temperature)
				continue
			}
